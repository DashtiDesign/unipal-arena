  ...
  if (s.results) {
    ...
    return (
      <div className="flex flex-col items-center gap-4 py-6 w-full">
        ...
        <p className="text-xl font-bold">{isDraw ? "Draw!" : iWon ? "You win!" : "You lose!"}</p>
    ...
    <div className="flex flex-col items-center gap-6 py-6 w-full">
      ...
      {iAnswered ? (
        <div className="flex flex-col items-center gap-2 py-4">
          <Spinner size="lg" />
          <p className="text-(--muted) text-sm">Waiting for opponent…</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-3 w-full">
          {s.options.map((opt) => (
            <button
              key={opt}
              className="border border-(--border) rounded-xl text-2xl font-mono h-16 min-h-[64px] bg-(--surface) hover:bg-(--surface-secondary) active:scale-95 transition-transform"
