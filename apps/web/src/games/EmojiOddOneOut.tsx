  ...
  if (s.results) {
    ...
    return (
      <div className="flex flex-col items-center gap-4 py-6 w-full">
        ...
        <p className="text-xl font-bold">{isDraw ? "Draw!" : iWon ? "You found it!" : "Too slow!"}</p>
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-4 w-full select-none">
      <div className="flex items-center justify-between w-full px-2">
        <p className="text-sm font-semibold">Which one is different?</p>
      ...
      {iAnswered ? (
        ...
        <div className="grid grid-cols-4 gap-2 w-full">
          {s.cells.map((emoji, i) => (
            <button
              key={i}
              className="aspect-square rounded-xl bg-(--surface-secondary) flex items-center justify-center text-3xl active:scale-90 transition-transform hover:bg-(--surface-tertiary)"
      ...
      {(s.answered[opponentId] ?? false) && !iAnswered && (
        <p className="text-xs text-(--muted)">Opponent answered…</p>
      )}
    </div>
  );
}
